<script lang="ts">
  ...
  function draw(dt: number) {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      ctx.beginPath();
      ctx.fillStyle =
        i % 2 ? `rgba(232,180,92,${0.35 + flicker * 0.4})` : `rgba(255,138,76,${0.3 + flicker * 0.45})`;
...
<canvas bind:this={canvas} {width} {height} class="pointer-events-none {className}" aria-hidden="true">
</canvas>
