<script lang="ts">
  ...
  async function handleClearAll() {
    const ok = await modal.confirm(
      'Clear All Memories',
      'Are you sure you want to erase all conversation memories, summaries, and episodic records? This cannot be undone.',
      'Clear All',
      'Cancel'
    );
    if (ok) {
            ...
            <CardContent class="p-3 flex flex-col gap-2">
              <div
                ...
                <div class="flex items-center gap-2">
                  <span class="text-gold">{'★'.repeat(item.importance)}</span>
                  <Button
                    variant="ghost"
                    size="icon"
                    class="h-6 w-6 text-muted-foreground hover:text-destructive text-xs"
                    onclick={() => deleteEpisodic(item.id)}>
                    ✕
                  </Button>
                </div>
              </div>
