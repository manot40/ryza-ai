...
<div class="h-full w-full flex flex-col bg-background/90 backdrop-blur-md overflow-hidden">
  ...
  <div class="flex items-center justify-between p-4 border-b border-border/40">
    <div class="flex items-center gap-2">
      ...
      <h2 class="text-lg font-bold text-gold">Daily Login</h2>
    </div>

    <div class="text-xs text-muted-foreground">
      Streak: <span class="font-bold text-gold">{streak}</span> Days
    </div>
  </div>
  ...
  <div class="flex-1 overflow-y-auto p-4 space-y-4">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
      {#each REWARDS as reward, idx}
        ...
        <Card
          class="border-border/50 bg-card/75 transition relative overflow-hidden {isCurrent ? 'ring-2 ring-gold bg-gold/10' : ''} {isClaimed ? 'opacity-60' : ''}"
        >
          <CardContent class="p-3 flex items-center justify-between">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm {isClaimed ? 'bg-muted text-muted-foreground' : isCurrent ? 'bg-gold text-background' : 'bg-card border border-border/60 text-foreground'}">
                {reward.day}
              </div>
            ...
            {:else if isCurrent}
              <span class="text-xs font-bold text-gold animate-pulse">
                Ready!
              </span>
            {/if}
          </CardContent>
    ...
    {#if claimNotice}
      <div class="p-3 rounded-md bg-leaf/20 border border-leaf/40 text-xs text-foreground font-medium text-center">
        {claimNotice}
      </div>
    {/if}
  </div>
  ...
  <div class="p-4 border-t border-border/40 bg-card/40">
    <Button
      class="w-full h-11 text-base font-bold {canClaim ? 'bg-gold text-background hover:bg-gold/90 shadow-md' : 'bg-muted text-muted-foreground'}"
      disabled={!canClaim}
      onclick={handleClaim}
    >
