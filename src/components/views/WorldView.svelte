<script lang="ts">
  ...
  import WorldMapView from './WorldMapView.svelte';
...
{#if viewMode === 'map'}
  <div class="relative w-full h-full flex-1 flex flex-col min-h-0 bg-[#0d1016]">
    <!-- Translucent Sleek Top Bar -->
    <header
      class="absolute top-0 inset-x-0 z-30 flex items-center justify-between px-3.5 py-2.5 bg-gradient-to-b from-[#0d1016]/95 via-[#0d1016]/60 to-transparent pointer-events-none">
      <div class="flex items-center gap-2 pointer-events-auto">
        {#if onClose}
          <Button
            variant="outline"
            size="icon"
            class="size-8 rounded-full border-white/20 bg-black/50 text-white/90 hover:bg-black/70 shadow-sm"
            onclick={onClose}
            title="Close Map">
            <XIcon class="size-4" />
          </Button>
        {/if}
        <span class="text-sm font-semibold text-[#f5ebd7] tracking-wide drop-shadow-sm">
          {world.areaName(selectedAreaId) || 'World Map'}
        </span>
      </div>
      ...
      <div class="flex items-center gap-2 pointer-events-auto">
        <Button
          variant="outline"
          size="sm"
          class="h-7 px-3 rounded-full text-xs font-medium border-white/20 bg-black/50 hover:bg-black/70 text-[#f5ebd7] gap-1.5 shadow-sm"
    ...
    <div class="w-full h-full flex-1 relative flex flex-col min-h-0">
      <WorldMapView
        {currentStageId}
        day={appState.day || 1}
        initialAreaId={selectedAreaId}
        onSelectStage={handleSelectStage}
        onToggleList={() => (viewMode = 'list')} />
    </div>
  </div>
{:else}
  <!-- View Header for List Mode -->
  <div
    class="flex items-center justify-between px-4 py-3 border-b border-border/40 bg-card/60 backdrop-blur-md">
    <div class="flex items-center gap-2">
      ...
      {/if}
      <h2 class="text-base font-bold text-gold">World Map</h2>
    </div>
              ...
              <CardContent class="p-3 flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <div class="flex flex-col min-w-0">
                    <span
                      class="text-sm font-semibold truncate {isCurrent ? 'text-gold' : 'text-foreground'}">
            ...
            <button
              ...
              <div class="flex flex-col flex-1 min-w-0">
                <div class="flex items-center gap-1.5">
                  ...
                  {#if !hasMet}
                    <span
                      class="text-[9px] px-1.5 py-0.2 bg-muted text-muted-foreground rounded-full shrink-0">
                      ?
                    </span>
                  {/if}
                </div>
