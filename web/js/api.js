...
(function (global) {
   ...
   */
  async function request(url, body, apiKey = '', timeoutMs = 120000) {
   ...
   * @returns {Promise<Blob | Record<string, unknown>>}
   */
  async function requestGet(url, apiKey = '', timeoutMs = 30000) {
    const headers = new Headers({ 'Content-Type': 'application/json' });
    if (apiKey) {
      headers.set('Authorization', `Bearer ${apiKey}`);
      headers.set('api-key', apiKey);
  ...
  var Api = {
    ...
    speak: function (text, lang, mode) {
      ...
      if (provider === 'qwen') return Api._qwenSpeak(text, lang, mode);
